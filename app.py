...
st.title("AIで株価予想")
st.write("AIを使って株価を予想するアプリです。")

image = Image.open('stock_predict.png')
st.image(image,use_column_width=True)

st.write("あくまでAIによる予測です（参考値）。こちらのアプリによる損害や過失については一切責任を負いません。")

st.header("株価銘柄のティッカーシンボルを入力してください")
stock_name=st.text_input("例:AAPL,FB,SFTBY ","AAPL")

stock_name=stock_name.upper()

link='https://search.sbisec.co.jp/v2/popwin/info/stock/pop6040_usequity_list.html'
st.markdown(link)
st.write("ティッカーシンボルは、上部LINKで確認できます。")

try:
    df_stock=yf.download(stock_name, start="2011-01-05" )
    df_stock.columns=df_stock.columns.droplevel(1)
    st.header(stock_name+'2022年1月5日から現在までの株価（USD）')
    st.write(df_stock)

    st.header(stock_name+"終わり値と14日間平気（USD）")
...
# st.line_chart(df_stock['change'].tail(100))
    ...
    st.header(stock_name+"ローソク足")
    st.plotly_chart(fig, use_column_width=True)

    df_stock['label']=df_stock['Close'].shift(-30)

    st.header(stock_name+'1ヶ月後を予測（USD）')
    def stock_predict():
    ...
    st.error('ティッカーシンボルが正しくありません。')
st.write('Copyright © 2021, Streamlit, Inc. All rights reserved.')
